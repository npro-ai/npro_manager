...
!define REVISION "rc1"
!define NAME "Wazuh"
!define SERVICE "NproSvc"

; output file
!ifndef OutFile
    !define OutFile "npro-agent-${VERSION}.exe"
!endif
...
Section "Wazuh Agent (required)" MainSec
    ...
    ; install files
    File npro-agent.exe
    File npro-agent-eventchannel.exe
    ...
    ; use appropriate version of "npro-agent.exe"
    ${If} ${AtLeastWinVista}
        Delete "$INSTDIR\npro-agent.exe"
        Rename "$INSTDIR\npro-agent-eventchannel.exe" "$INSTDIR\npro-agent.exe"
    ${Else}
        Delete "$INSTDIR\npro-agent-eventchannel.exe"
    ${Endif}
...
Section "Uninstall"
    ...
    ServiceUninstall:
        nsExec::ExecToLog '"$INSTDIR\npro-agent.exe" uninstall-service'
        Pop $0
        ${If} $0 <> 1
            MessageBox MB_ABORTRETRYIGNORE|MB_ICONSTOP "$\r$\n\
